...
Future<void> main() async {
  ...
  try {
    ...
    // Nếu duplicate-app thì bỏ qua, không làm gì cả
  }
  ...
  // FCM: Lấy token và lắng nghe thông báo
  FirebaseMessaging messaging = FirebaseMessaging.instance;
  String? token = await messaging.getToken();
  print('FCM Token: ' + (token ?? '')); // In token ra debug console

  FirebaseMessaging.onMessage.listen((RemoteMessage message) {
    print(
        'Nhận được thông báo foreground: ${message.notification?.title} - ${message.notification?.body}');
  });
  FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) {
    print('Người dùng mở app từ thông báo: ${message.notification?.title}');
  });
